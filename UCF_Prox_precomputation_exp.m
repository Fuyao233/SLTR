% To prove that Prox is not sensitive to the epsilon
% Method: choose different epsilon {0.01, 0.05, 0.1, 0.5, 1.0, 10.0,  min(eig(X^TX)) } and compare the number of iteration and iteration time with same (Tau, Lambda)
...
disp('===== Prox_Remurs =====')
    ...
    for idx_e = 1:length(epsilon_list)
        ...
        save(sprintf('Prox_precomputation_res/Prox_ROC_g%d_epsilon%d.mat', num, idx_e), 'Prox_ROC')
    end
