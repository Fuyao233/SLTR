function [] = each_iter_exp()
   ...
   %% Prox_Remurs
    ...
    cvLambda = 0.0001; 
    cvEpsilon = 0.1;
    fprintf('cv Tau is %.4f \n',cvTau)
    fprintf('cv Lambda is %.4f \n',cvLambda)
    fprintf('cv Epsilon is %.4f \n',cvEpsilon)
    
    
    [err_mat, estimatedW, ~, estimation] = iter_Prox_Remurs(double(invertX), matrixY, cvTau, cvLambda, cvEpsilon, rho, maxIter, minDiff);
    ...
    save('Prox_Remur_estimation.mat', 'estimation');
    ...
    cvAlpha = 0.005;
    cvBeta = 0.005;
    fprintf('cv Alpha is %.4f \n',cvAlpha)
    fprintf('cv Beta is %.4f \n',cvBeta)
    [err_mat, estimatedW, ~, ~, estimation] = iter_Remurs(X, double(invertX), matrixY, cvAlpha, cvBeta, epsilon, iter);
    predY = ttt(tensor(X), tensor(estimatedW), 2:M+1, 1:M);
    fprintf('Prediction Error: %f\n',norm(tensor(predY.data, [options.N, 1]) - tensor(matrixY)) / options.N)
    error = reshape(W.data, options.p)-estimatedW;
    fprintf('Estimation error: %f\n',norm(tensor(error)) /norm(tensor(W)))
    save('Remurs_iter.mat','err_mat');
    save('Remurs_estimation.mat','estimation');
    
    
function [err_mat, estimatedW, errSeq, estimation] = iter_Prox_Remurs(X, Y, tau, lambda, epsilon, rho, maxIter, minDiff)
    %% Initializations
    p = size(X);
    ...
    est_W1=zeros([maxIter 20 20 5]);
    est_W2=zeros([maxIter 20 20 5]);
    est_W3=zeros([maxIter 20 20 5]);
    for m=1:M
        ...
        for t=1:maxIter
            ...
            if m == 1
                est_W1(t,:,:,:) = Fold(W_m, p(1:M), m);
            end
            if m == 2
                est_W2(t,:,:,:) = Fold(W_m, p(1:M), m);
            end
            if m == 3
                est_W3(t,:,:,:) = Fold(W_m, p(1:M), m);
            end
    ...
    estimation.est_W1 = est_W1;
    estimation.est_W2 = est_W2;
    estimation.est_W3 = est_W3;
    

function [err_mat, tW, errList, time, estimation] = iter_Remurs(X, tX, y, alpha, beta, epsilon, max_iter)
...
estimation=zeros([max_iter 20 20 5]);
