...
for testFold = 1:numFold
    fprintf('testFold: %d\n\n', testFold)
    %% Train--test
    indexTest  = indexTotal((testFold-1)*numUnit+1 : testFold*numUnit);
    ...
    best = cross_validation_ENet(TrainData_1D', yTrain, setting);
    
    %% Train the model
    opt.rsL2 = best.beta;
    %opt.nFlag = 1;
    [w, funVal] = LeastR(TrainData_1D', yTrain, best.alpha, opt);
    ...
    save([ResultFolder '/ENet_P',num2str(subject),'_F',num2str(testFold)], 'w', 'fold');
end
...
save([ResultFolder '/ENet_P',num2str(subject),'_Results'], 'result');
