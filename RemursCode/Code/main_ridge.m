...
for testFold = 1:numFold
    ...
    best = cross_validation_ridge(TrainData_1D', yTrain, setting);
    
    %% Train the model
    opt.rsL2 = best.beta;
    %opt.nFlag = 1;
    [w, funVal] = LeastR(TrainData_1D', yTrain, 0, opt);
    ...
    save([ResultFolder '/Ridge_P',num2str(subject),'_F',num2str(testFold)], 'w', 'fold');
end
...
save([ResultFolder '/Ridge_P',num2str(subject),'_Results'], 'result');
