...
XY = Xt' * Y;
diag_XX = sum(Xt.^2, 1);
[J_max, ind_max] = max(2*abs(XY)-abs(XY));
if J_max~=0
...
for i=1:N
    I{1,i} = index(i);
    %w{t,i} = zeros(1,dim(i)); %[Jiaqi Zhang] Q: why zeros?
    w{t,i} = ones(1,dim(i)); %[Jiaqi Zhang] revised
    w{t,i}(1,I{1,i}) = 1;
end
sigma(t) = epsilon;
w{t,1} = sign(s) * w{1,1};
...
while 1
    ...
    W = wt(:);
    if norm(LastW - W)/ norm(W) >= absW && t<1000 %[Jiaqi Zhang] revised
