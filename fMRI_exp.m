...
% for different pars cyh 2022.10.1
% cvAlpha = [10^-3, 5*10^-3, 10^-2, 5*10^-2, 10^-1, 5*10^-1, 10^0, 5*10^0];
% cvBeta = [10^-3, 5*10^-3, 10^-2, 5*10^-2, 10^-1, 5*10^-1, 10^0, 5*10^0];
% best_AUC = 0;
% best_par = [0, 0];
% for al = 1:length(cvAlpha)
%     for be = 1:length(cvBeta)
%         [estimatedW, errList] = Remurs(double(trainInvertX), matrixY, cvAlpha(al), cvBeta(be), epsilon, iter);
%         fprintf('Elapsed time is %.4f sec\n',t)
%         predY = ttt(tensor(testX), tensor(estimatedW), 2:M+1, 1:M);
%         predY = reshape(double(predY),[N-splitPoint 1]);
%         [rocX, rocY, rocT, AUC] = perfcurve(double(testY), sigmoid(predY), 1);
...
%             end
%         end
%         fprintf('Alpha: %f, Beta: %f', cvAlpha(al), cvBeta(be));
%         fprintf('The correct rate of Prox_Remurs is %f\n', correct_cnt/length(predY));
...
%            best_par = [al, be];
%         end
%         fprintf('The best AUC value of Remurs is %f, best_par = %f,%f !', best_AUC, best_par(1), best_par(2));
%         % save the ROC curve
%         Remur_ROC.X = rocX;
%         Remur_ROC.Y = rocY;
%         Remur_ROC.T = rocT;
...
%         save(sprintf('fMRI_data/P%d_res/Remur_ROC.mat',case_num), 'Remur_ROC')
%     end
% end
...
% save(sprintf('fMRI_data/P%d_res/SURF_Roc.mat', case_num), 'SURF_Roc')
...
predY = reshape(double(predY), [size_predY(1) 1]);
[rocX, rocY, rocT, AUC] = perfcurve(double(testY), sigmoid(predY), 1);
...
% save(sprintf('fMRI_data/P%d_res/Lasso_ROC.mat',case_num), 'Lasso_ROC')
disp('Finish time')
res_scatter(predY, testY);
%% ENet
% % parameter settings
% addpath('GLMNET/');
% disp('===== Elasticnet =====')
...
% ENet.true_Y = testY;
% ENet.pred_Y = predY;
% save(sprintf('fMRI_data/P%d_res/ENet_ROC.mat',case_num), 'ENet')
% disp('Finish time')
